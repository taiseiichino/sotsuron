...
class Policy1:
    def __init__(self, grid, eta_size=2):
        ...
        for nexteta_index in range(self.eta_size):
            ...
            theta1[3][nexteta_index] = np.array([np.nan, 1, 1, 1])
            theta1[4][nexteta_index] = np.array([np.nan, 1, 1, 1])
        for nexteta_index in range(self.eta_size):
            theta1[5][nexteta_index] = np.array([np.nan, np.nan, 1, 1])
        for nexteta_index in range(self.eta_size):
            theta1[6][nexteta_index] = np.array([1, 1, 1, np.nan])
            theta1[12][nexteta_index] = np.array([1, 1, 1, np.nan])
            theta1[18][nexteta_index] = np.array([1, 1, 1, np.nan])
            theta1[24][nexteta_index] = np.array([1, 1, 1, np.nan])
            theta1[30][nexteta_index] = np.array([1, 1, 1, np.nan])
            theta1[36][nexteta_index] = np.array([1, 1, 1, np.nan])
        for nexteta_index in range(self.eta_size):
            theta1[11][nexteta_index] = np.array([1, np.nan, 1, 1])
            theta1[17][nexteta_index] = np.array([1, np.nan, 1, 1])
            theta1[23][nexteta_index] = np.array([1, np.nan, 1, 1])
            theta1[29][nexteta_index] = np.array([1, np.nan, 1, 1])
            theta1[35][nexteta_index] = np.array([1, np.nan, 1, 1])
            theta1[41][nexteta_index] = np.array([1, np.nan, 1, 1])
        for nexteta_index in range(self.eta_size):
            theta1[42][nexteta_index] = np.array([1, 1, np.nan, np.nan])
        for nexteta_index in range(self.eta_size):
            theta1[47][nexteta_index] = np.array([1, np.nan, np.nan, 1])
        for nexteta_index in range(self.eta_size):
            theta1[43][nexteta_index] = np.array([1, 1, np.nan, 1])
            theta1[44][nexteta_index] = np.array([1, 1, np.nan, 1])
            theta1[45][nexteta_index] = np.array([1, 1, np.nan, 1])
            theta1[46][nexteta_index] = np.array([1, 1, np.nan, 1])
    ...
    def __init__(self, grid, eta_size=2):
        ...
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                ...
                theta2[3][eta_index][nexteta_index] = np.array([np.nan, 1, 1, 1])
                theta2[4][eta_index][nexteta_index] = np.array([np.nan, 1, 1, 1])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[5][eta_index][nexteta_index] = np.array([np.nan, np.nan, 1, 1])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[6][eta_index][nexteta_index] = np.array([1, 1, 1, np.nan])
                theta2[12][eta_index][nexteta_index] = np.array([1, 1, 1, np.nan])
                theta2[18][eta_index][nexteta_index] = np.array([1, 1, 1, np.nan])
                theta2[24][eta_index][nexteta_index] = np.array([1, 1, 1, np.nan])
                theta2[30][eta_index][nexteta_index] = np.array([1, 1, 1, np.nan])
                theta2[36][eta_index][nexteta_index] = np.array([1, 1, 1, np.nan])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[11][eta_index][nexteta_index] = np.array([1, np.nan, 1, 1])
                theta2[17][eta_index][nexteta_index] = np.array([1, np.nan, 1, 1])
                theta2[23][eta_index][nexteta_index] = np.array([1, np.nan, 1, 1])
                theta2[29][eta_index][nexteta_index] = np.array([1, np.nan, 1, 1])
                theta2[35][eta_index][nexteta_index] = np.array([1, np.nan, 1, 1])
                theta2[41][eta_index][nexteta_index] = np.array([1, np.nan, 1, 1])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[42][eta_index][nexteta_index] = np.array([1, 1, np.nan, np.nan])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[47][eta_index][nexteta_index] = np.array([1, np.nan, np.nan, 1])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[43][eta_index][nexteta_index] = np.array([1, 1, np.nan, 1])
                theta2[44][eta_index][nexteta_index] = np.array([1, 1, np.nan, 1])
                theta2[45][eta_index][nexteta_index] = np.array([1, 1, np.nan, 1])
                theta2[46][eta_index][nexteta_index] = np.array([1, 1, np.nan, 1])
...
"""
    ...
    def update_pi1_SGD_distributed(self, agent_index, P, z, other_agent1, other_agent2, other_agent3, other_agent4, other_agent5, learning_rate=None, kappa=None):
        """
        分散最適化用の推定値の更新
        方策Policy1のネットワークの更新を行う（動的学習率を適用）
        :param learning_rate: 新しい学習率（指定がなければデフォルトを使用
        ...
        """for i in range(self.state_size):
            print(f'self.pi1.theta1:{self.pi1.softmax_probs()[i]}')
"""
    def update_pi2_SGD_distributed(self, agent_index, P, z, other_agent1, other_agent2, other_agent3, other_agent4, other_agent5, learning_rate=None, kappa=None):
        """
        方策Policy1のネットワークの更新を行う（動的学習率を適用）
        :param learning_rate: 新しい学習率（指定がなければデフォルトを使用）
        """
