...
register(
    ...
    kwargs={'grid': custom_grid, 'slip_prob': 0.1},  # 環境に渡すパラメータ
)
...
episodes = 30001
agent1 = Agent(kappa=0.5, eta_size=env.eta_space.n, Lambda=0.5, alpha_risk=0.05, grid_shape=[env.n_rows, env.n_cols], eta_space=env.cost_space)#action_size,state_sizeはint型で渡している
agent2 = Agent(kappa=0.5, eta_size=env.eta_space.n, Lambda=0.5, alpha_risk=0.05, grid_shape=[env.n_rows, env.n_cols], eta_space=env.cost_space)
agent3 = Agent(kappa=0.5, eta_size=env.eta_space.n, Lambda=0.5, alpha_risk=0.05, grid_shape=[env.n_rows, env.n_cols], eta_space=env.cost_space)
