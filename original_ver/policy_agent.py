...
class Policy1:
    def __init__(self, eta_size=2):
        ...
        for nexteta_index in range(self.eta_size):
            theta1[12][nexteta_index] = np.array([1, 1, np.nan, np.nan])
        for nexteta_index in range(self.eta_size):
            theta1[15][nexteta_index] = np.array([1, np.nan, np.nan, 1])
        for nexteta_index in range(self.eta_size):
            theta1[13][nexteta_index] = np.array([1, 1, np.nan, 1])
            theta1[14][nexteta_index] = np.array([1, 1, np.nan, 1])
        #self.theta1 = theta1
        """
...
class Policy2:
    def __init__(self, eta_size=2):
        ...
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[12][eta_index][nexteta_index] = np.array([1, 1, np.nan, np.nan])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[15][eta_index][nexteta_index] = np.array([1, np.nan, np.nan, 1])
        for eta_index in range(self.eta_size):
            for nexteta_index in range(self.eta_size):
                theta2[13][eta_index][nexteta_index] = np.array([1, 1, np.nan, 1])
                theta2[14][eta_index][nexteta_index] = np.array([1, 1, np.nan, 1])
...
class Agent:
    def __init__(self, kappa, eta_size, Lambda, alpha_risk, grid_shape, eta_space, action_size=4, state_size=16, gamma=0.9999, lr=0.002, epsilon=1e-8):
        """
        エージェントの初期化
        ...
        self.cost_memory = []
        self.prob_memory = []
        self.pi1 = Policy1(eta_size=self.eta_size)
        self.pi2 = Policy2(eta_size=self.eta_size)
