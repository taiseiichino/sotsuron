...
from policy_agent_comp import Agent
import math
...
episodes = 20001
agent = Agent(kappa=0.5, eta_size=env.eta_space.n,grid=custom_grid ,Lambda=0.0, alpha_risk=0.05, grid_shape=[env.n_rows, env.n_cols], eta_space=env.cost_space)#action_size,state_sizeはint型で渡している
cost_history = []
cost_list = []
p1_grad_history = []
p2_grad_history = [] 
...
for episode in range(episodes):
    ...
    #print('update pi1')
    p1_grad_history.append(agent.update_pi1_SGD(learning_rate=step_size, kappa=kappa))
    #print('update pi2')
    p2_grad_history.append(agent.update_pi2_SGD(learning_rate=step_size, kappa=kappa))
    #メモリーをクリア
    agent.memory = []
    data = {}
...
dfp1 = pd.DataFrame(p1_grad_history)
dfp2 = pd.DataFrame(p2_grad_history)
# ベースフォルダ
base_folder = "数値データ"
...
dfp1.to_csv(os.path.join(output_folder, 'p1_grad_history.csv'), index=False)
dfp2.to_csv(os.path.join(output_folder, 'p2_grad_history.csv'), index=False)
